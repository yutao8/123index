...
class Amoli
{
	...
	public function getPan123List($config,$dirId=0,$lastFileId=0,$limit=100): array {
		$sdk=pan123Client::boot($config['client_id'],$config['client_secret']);
		$rootDirId=$dirId?:$config['root_dir'];
		
		$cacheKey=md5('list_v2'.$config['client_id'].serialize([$rootDirId,$lastFileId,$limit]));
		$cacheData=$sdk->file->cache($cacheKey);
		if(empty($cacheData)){
			$res= $sdk->file->list_v2($rootDirId,$lastFileId,$limit);
			empty($res['fileList']) || $r=$sdk->file->cache($cacheKey,$res,time()+$config['time2']??360);
		}else{
			$res=$cacheData;
		}
		
		$result=[];
		if(empty($res['fileList'])){
	...
	public function getPan123Url($config,$fileId){
		$sdk=pan123Client::boot($config['client_id'],$config['client_secret']);
		$signKey=$config['sign_key']??'';
		$uid=$config['uid']??0;
		if(!empty($signKey) && empty($uid)){
			return ['code' => 2, 'msg' => '请填写UID'];
		}
		$cacheKey=md5('link_url'.$config['client_id'].$fileId);
		$cacheData=$sdk->link->cache($cacheKey);
		if(empty($cacheData)){
			$url=$sdk->link->url((int)$fileId);
			empty($url) || $sdk->link->cache($cacheKey,$url,time()+$config['time2']??86400);
		}else{
			$url=$cacheData;
		}
		if(empty($url)){
			return ['code' => 2, 'msg' => $sdk->getError('msg')];
		}
		if($signKey){
			$url=$sdk->link->sign($url,$signKey,(int)$uid,(int)$config['time']?:3600);
		}
		return ['code' => 1, 'msg' => '获取成功！', 'data' => ['url' => $url]];
	}
