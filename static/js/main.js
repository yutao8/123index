    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0, Max = item.length; i < Max; i++) {
                if (item[i].type == 'wjj') {
                    var name = '<a href="javascript:;" data-id="'+item[i]['id']+'" onclick="getList(\'ml\',\'' + dir + '\',this)">' + item[i].name + '</a>';
                } else {
                    var name = '<a href="javascript:;" data-id="'+item[i]['id']+'" onclick="downVerify(\'' + item[i].type + '\',\'' + item[i].name + '\', \'' + dir + item[i].name + '\',this)">' + item[i].name + '</a>';
                }
            ...
            switch (type) {
                case 'nav':
                    ...
                    break;
                case 'ml':
                    var nav = '<a  class="breadcrumb-item" data-id="'+id+'" href="javascript:;"  onclick="getList(\'nav\',\'' + dir + '\',this)">' + $(ojb).text() + '</a>';
                    $('#nav').append(nav);
                    break;
                case 'hash':
                    var nav = '<a class="breadcrumb-item"></a>',
...
// 下载验证
function downVerify(type, title, dir,obj) {
    if (verify) {
        layer.open({
            type: 1,
        ...
        Preview(type, title, dir,{id:$(obj).data('id')});
    }
}
// 下载、预览文件
